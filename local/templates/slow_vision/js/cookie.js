...
function changeKerning(val){
    var HTML = $('html');

    switch (val) {
        case 'normal':
            HTML
                .removeClass('special-aaKerning-big')
                .removeClass('special-aaKerning-extra');
            break;
        case 'big':
            HTML
                .removeClass('special-aaKerning-extra')
                .addClass('special-aaKerning-big');
            break;
        case 'extra':
            HTML
                .removeClass('special-aaKerning-big')
                .addClass('special-aaKerning-extra');
            break;
    }
    if (getCookie('js-kerning') !== val) {
        setCookie('js-kerning', val);
    }
}
...
$(function() {
    ...
    $('.aaKerning .aaKerning-wrapper a').on('click', function () {
        changeKerning($(this).data('aa-kerning'));
        $('.aaKerning .aaKerning-wrapper a').removeClass('a-current');
        $(this)
            .addClass('a-current');
    });
    ...
    if( getCookie("js-kerning") != null )
        changeKerning( getCookie("js-kerning") );
});